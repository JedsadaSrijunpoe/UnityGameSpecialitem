...
public class SpawnManager : NetworkBehaviour
{
    ...
    [ServerRpc(RequireOwnership = false)]
    public void SpawnGhostBotServerRpc()
    {
        GameObject ghostBot = (GameObject)Instantiate(Network_Prefab_List[2]);
        ghostBot.SetActive(true);
        ghostBot.transform.position = new Vector3(11.47f, -19.39f, 0f);
        ghostBot.GetComponent<NetworkObject>().Spawn();
    }

    [ServerRpc]
    public void DestroyAllGhostBotServerRpc()
    {
        GameObject[] ghosts = GameObject.FindGameObjectsWithTag("GhostLabel");
    ...
    [ServerRpc(RequireOwnership = false)]
    public void DestroyGhostBotServerRpc()
    {
        GameObject[] ghosts = GameObject.FindGameObjectsWithTag("GhostLabel");
        if (ghosts.Length > 0)
        {
            int ghostIndex = Random.Range(0, ghosts.Length + 1);
            Destroy(ghosts[ghostIndex]);
        }
    }

    [ServerRpc]
    public void FreezeAllGhostsServerRpc()
    {
        Debug.Log("Ghost is frozen");
        GameObject[] ghosts = GameObject.FindGameObjectsWithTag("GhostLabel");
        foreach (GameObject ghost in ghosts)
        {
            AIPath ghostAI = ghost.GetComponent<AIPath>();
            ghostAI.canMove = false;
            StartCoroutine(DeactivateFreeze(ghostAI));
        }
    }

    public IEnumerator DeactivateFreeze(AIPath ghostAI)
    {
        yield return new WaitForSeconds(3);
        Debug.Log("Ghost is unfrozen");
        ghostAI.canMove = true;
    }
}
